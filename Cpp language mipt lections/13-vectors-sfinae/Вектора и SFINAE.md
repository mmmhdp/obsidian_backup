...
// могут произойти реаллокации посреди цикла
...
Амортизированная алгоритмическая сложность для `O(n)` обозначается как `O(n+)` или `O(n)+`.
...
Обычно мы хотим делать конструктор шаблонным, но не как остальные методы.
