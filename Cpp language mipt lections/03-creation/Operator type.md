...
быть неявно преобразован. 

Для этого мы пишем `operator type`:
```cpp
struct Foo {
  int x_;
...
На преобразовании в другой тип `explicit` имеет семантику запрета неявного преобразования, оставляя возможность только для явного перевода в другой тип:
```cpp
struct Foo {
  int x_;
  explicit Foo(int x) : x_(x) {}
